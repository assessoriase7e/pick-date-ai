...
const BentoCard = ({ name, className, background, iconName, description, href, cta, ...props }: BentoCardProps) => {
  ...
  const IconComponent =
    (LucideIcons as unknown as Record<string, React.ElementType>)[iconName] || LucideIcons.HelpCircle; // Fallback para um ícone padrão
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "bg-background",
        // dark styles
        "transform-gpu border border-accent",
        className
      )}
      {...props}
    >
      ...
      <motion.div
        ...
        initial={{ y: 0 }}
        animate={isMobile ? { y: isInView ? -80 : 0, scale: isInView ? 1.05 : 1 } : { y: 0, scale: 1 }}
        transition={{ duration: 0.5 }}
      >
        <IconComponent className="h-12 w-12 origin-left transform-gpu text-accent stroke-[1px]" />
